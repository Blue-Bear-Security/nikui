...
class HotspotCalculator:
    ...
    def calculate(self, findings):
        ...
        return sorted(file_stats.items(), key=lambda x: x[1]['hotspot_score'], reverse=True)
...
class HtmlReporter:
    ...
    def render(self, sorted_files, findings, html_path):
        ...
        html_content = template.replace("{{weights}}", json.dumps(self.config.get('stench_weights', {})))
...
def generate_reports(repo_path, json_path, html_path, config_path):
    if not os.path.exists(config_path):
        print(f'Error: {config_path} missing'); sys.exit(1)
