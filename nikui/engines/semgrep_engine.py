...
class SemgrepEngine:
    ...
    def categorize_finding(self, check_id, semgrep_severity, semgrep_category=None):
        ...
        is_security = (
            "security" in check_id_lower or 
            "security" in category_lower or 
            semgrep_severity == "ERROR" or 
            "audit" in check_id_lower
        )
        if is_security:
        ...
        if any(kw in check_id_lower for kw in ["best-practice", "correctness", "convention"]):
    ...
    def run_stage(self, temp_dir):
        ...
        excludes = " ".join([f"--exclude {d}/" for d in self.config["exclusions"]["directories"]])
        
        command = f"semgrep scan {configs} {excludes} --json > {shlex.quote(output_file)}"
        try:
