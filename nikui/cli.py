...
def main():
    ...
    smell_parser.add_argument("--output", default=None, help="Output JSON file (defaults to results/ folder)")
    ...
    report_parser.add_argument("--json", default=None, help="Input JSON findings (defaults to latest in results/)")
    ...
    if args.command == "smell":
        ...
        sys.argv = [sys.argv[0], args.repo_path, "--config", args.config, "--output", output_arg]
        smell_main()
        ...
        sys.argv = [sys.argv[0], args.repo_path, "--json", json_input, "--html", html_output, "--config", args.config]
        report_main()
    else:
        parser.print_help()
